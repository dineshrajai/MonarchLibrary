...
Imports System.IO
Public Class _customMasterForm
    ...
    Public Sub btncancel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btncancel.Click
        ...
        ModGlobalVar._writer.WriteLine(My.Computer.Name & "    cancel  Changes " & _bindDatasourceProcess.List(0)(_sPrimaryKey).ToString & "  by " & ModGlobalVar.gv_username & "    " & Me.Name & " for " & ModGlobalVar.gv_fileno)
        ModGlobalVar._writer.Flush()
        If _bindDatasourceProcess.List.Count = 0 Then

        Else
            fillProcess(Val(_bindDatasourceProcess.List.Item(0)(_sPrimaryKey).ToString))
            'tabcontrol.SelectedTabPage = tablist
            'SendKeys.Send("%(E)")
        End If
        'tabcontrol.SelectedTabPage = tablist
        'ModClass.EnableAllControls(tabProcess)
        'PanelControl2.Enabled = False
        GridView1.Focus()
        btnAdd.Enabled = True
        btnEdit.Enabled = True
    ...
    Private Sub GridView1_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles GridView1.KeyPress
        '   If GridView1.IsFindPanelVisible = False And ModClass.isalpahanum(e.KeyChar) Then
        If ModClass.isalpahanum(e.KeyChar) Then
